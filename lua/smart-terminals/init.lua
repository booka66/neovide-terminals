	...
	if term.window and vim.api.nvim_win_is_valid(term.window) then
		vim.schedule(function()
			pcall(function()
				...
				local centered_title = string.rep(
					" ",
					math.max(0, math.floor((win_width - string.len(title_text)) / 2))
				) .. title_text
...
M.send_line_to_terminal = function()
	...
	if term.use_tmux and term.tmux_session then
		...
		local cmd = "tmux send-keys -t "
			.. vim.fn.shellescape(term.tmux_session)
			.. " "
			...
			if term and term.window and vim.api.nvim_win_is_valid(term.window) then
				vim.schedule(function()
					pcall(function()
						...
						if
							term.title_win
							and vim.api.nvim_win_is_valid(term.title_win)
							and term.title_buf
							and vim.api.nvim_buf_is_valid(term.title_buf)
						then
...
M.setup = function(opts)
	...
	local function map_keys(keys, func, opts)
		for _, key in ipairs(keys) do
			local modes = key:match("<leader>") and "n" or { "n", "i", "v", "t" }
			map(modes, key, func, opts)
		end
	end

	-- Single mapping to toggle current terminal
